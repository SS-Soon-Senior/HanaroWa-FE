...
describe('formatter 유틸', () => {
  ...
  describe('formatLessonTime', () => {
    it('알려진 요일 라벨로 수업 시간을 포맷해야 한다', () => {
      expect(
        formatLessonTime('2024-03-15~2024-04-15 mon-wed 09:00-10:00')
      ).toBe('월, 수 09:00-10:00');
      expect(
        formatLessonTime('2024-03-15~2024-04-15 tue-thu 14:00-16:00')
      ).toBe('화, 목 14:00-16:00');
      expect(
        formatLessonTime('2024-03-15~2024-04-15 weekend 10:00-12:00')
      ).toBe('토, 일 10:00-12:00');
    });

    it('알 수 없는 요일 라벨을 처리해야 한다', () => {
      expect(
        formatLessonTime('2024-03-15~2024-04-15 custom-day 09:00-10:00')
      ).toBe('custom-day 09:00-10:00');
    });
  ...
  describe('calculateDurationInWeeks', () => {
    it('주 단위로 기간을 계산해야 한다', () => {
      expect(
        calculateDurationInWeeks('2024-03-01~2024-03-15 mon-wed 09:00-10:00')
      ).toBe('2주');
      expect(
        calculateDurationInWeeks('2024-03-01~2024-03-29 tue-thu 14:00-16:00')
      ).toBe('4주');
    });

    it('잘못된 날짜를 처리해야 한다', () => {
      expect(calculateDurationInWeeks('invalid~date mon-wed 09:00-10:00')).toBe(
        '기간 계산 불가'
      );
    });
